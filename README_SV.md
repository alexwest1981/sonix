...
### 11. 🔌 Plugin-hanterare — 🟡 Katalog + opt-in CLAP/VST3/VST2-värd
...
* 🔜 **Not:** **VST3- och VST2**-moduler kan nu laddas, inspekteras och **spelas** (opt-in `--features plugin-host`), precis som CLAP. **LV2** är fortfarande endast katalog. Plugin-GUI:t kräver en riktig X-display (kan inte visas headless). Se **[Plugin-stöd — nuläget](#-plugin-stöd--nuläget)**.
...
> **Kort svar: katalog + opt-in CLAP-, VST3- och VST2-laddning, inspektion, per-spår-ljudprocessning, projektlagrad state, plugin-GUI i eget X11-fönster och en out-of-process-sandbox med krasch/omstart och delat-minne-ljudtransport.** Sonix kan *hitta och katalogisera* alla format och — när det byggs med `--features plugin-host` — *ladda* en native **CLAP-, VST3- eller VST2**-plugin, läsa dess parametrar, **köra ljud genom den på ett stämspår** med delay-kompensation, **spara/återställa dess state med projektet**, **ladda pluginens egna presets** (CLAP), **öppna pluginens eget gränssnitt i ett X11-fönster som delar samma instans som processar ljudet** (CLAP), **köra pluginens ljudprocessning i en separat process över delat minne** samt **starta om den automatiskt vid krasch**. LV2, MIDI-instrument-hosting och att verifiera en riktig Wine/yabridge-brygga återstår.
...
* **VST3-laddning, inspektion, ljud, state & PDC (opt-in, `--features plugin-host`):** en **handrullad minimal VST3-ABI** (ingen SDK) laddar en `.vst3`-modul (ELF eller bundle), skapar komponenten via fabriken och läser dess parametrar (namn, enheter, steg, flaggor). En levande `VstProcessor` kör sedan `IAudioProcessor::process` på riktiga stereoblock, rapporterar `getLatencySamples` till motorns PDC, driver parametrar via `IEditController` + `IParameterChanges` och sparar/laddar state via en host-ägd `IBStream` (`IComponent::getState`/`setState` + `IEditController::setComponentState`). Särskilt relevant eftersom **yabridge producerar VST3/VST2**, inte CLAP.
* **VST2-laddning, inspektion, ljud, state & PDC (opt-in, `--features plugin-host`):** en **handrullad minimal VST 2.4-ABI** (ingen SDK) `dlopen`:ar en `.so`-brygga (t.ex. yabridge:s `~/.vst/yabridge/*.so`), anropar `VSTPluginMain`, validerar `AEffect`-magin och svarar på pluginens `audioMaster`-callback. En levande `Vst2Processor` kör sedan `processReplacing` på riktiga stereoblock, läser parametrar via `getParameter`/`effGetParamName`/`effGetParameterProperties`, rapporterar `initialDelay` till motorns PDC, nollställer via `effMainsChanged` och sparar/laddar state som en program-chunk (`effGetChunk`/`effSetChunk`). `.so`-sökvägar (utan `.vst3`/`.clap`) routas automatiskt till denna backend.
...
2. Wine/yabridge-vägen för FL Studio & Windows-VST (Fas 4.6): **VST3-modul/ABI/laddning/inspektion (4.6a), VST3-ljud/state/PDC (4.6b) och VST2-ABI/ljud/state/PDC (4.6c) är klara**; att köra en **riktig** yabridge-brygga (kräver Wine + display) återstår.
3. LV2-laddning — CLAP, VST3 och VST2 kan spela ljud så långt.
...
**Därför:** en CLAP-, **VST3- eller VST2-effekt** är nu användbar för ljud i Sonix när den byggs med `--features plugin-host` — ladda den, sätt den som insert på ett stämspår med "▶ Ladda in", öppna dess GUI med "🪟 Öppna GUI" (CLAP), och dess state överlever projektets spara/ladda. Kraschisolering via en separat process med **delat-minne-ljud** finns nu (med automatisk omstart); LV2, instrument-hosting och att verifiera en riktig Wine/yabridge-brygga återstår.
...
Katalogisering av FL Studio-`.fst`-presets samt CLAP-, VST3-, VST2-, LV2- och Wine/Yabridge-pluginplatser (skanning & verifiering — CLAP-, **VST3- och VST2-**processning opt-in, LV2-hosting ännu inte implementerad).
