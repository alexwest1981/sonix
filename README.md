...
* **Crash isolation in a separate process:** **"🧪 Sandbox inspect"** runs the plugin in its own process (the same binary re-executed with `--plugin-sandbox-worker`) over a length-prefixed JSON protocol and reads its info + parameters there; a supervisor **automatically restarts it on a crash** (up to three attempts) so Sonix does not go down. (Audio still runs in-process; shared-memory transport comes in Fas 4.5b.)
...
| Plugin hosting (VST3/CLAP/LV2/VST2) | 🟡 Partial | CLAP load + parameter inspection + per-track audio processing with PDC + state/preset save-load + GUI in its own X11 window + separate-process sandbox with crash restart (opt-in `--features plugin-host`); MIDI instrument routing & shared-memory audio transport still missing |

**Overall:** roughly **90–95 %** of the features advertised in the UI are genuinely implemented and wired to the audio engine. The largest outstanding piece is **plugin hosting** (CLAP load + parameter inspection + per-track audio processing with PDC + state/preset save-load + GUI in its own X11 window + separate-process sandbox with crash restart work opt-in; MIDI instrument routing and shared-memory audio transport remain); neural stem separation is implemented (opt-in `--features neural` + a user-supplied HTDemucs ONNX).
...
> **Short answer: catalogue + opt-in CLAP loading, inspection, per-track audio processing, project state/preset persistence, a plugin GUI in its own X11 window and an out-of-process sandbox with crash restart.** Sonix can *find and catalogue* every format, and — when built with `--features plugin-host` — *load* a native CLAP plugin, read its parameters, **run audio through it on a stem track with plug-in delay compensation**, **save/restore its state with the project**, **load the plugin's own presets**, **open the plugin's own UI in an X11 window that shares the same instance that processes audio** and **inspect/monitor a plugin in a separate process that is automatically restarted on a crash**. Shared-memory audio transport (Fas 4.5b) remains.
...
* **Out-of-process sandbox (crash restart):** **"🧪 Sandbox inspect"** runs the plugin in a **separate process** (the same binary re-executed with `--plugin-sandbox-worker`) over a length-prefixed JSON protocol and reads its info + parameters there. A supervisor monitors the process and **automatically restarts it on a crash** (up to three attempts) before giving up, so a crashing plugin does not take Sonix down. (Audio still runs in-process; shared-memory transport is Fas 4.5b.)

**What is still missing to fully host plugins (🔜)**
1. Shared-memory audio transport in the sandbox so the audio processing itself runs in the separate process (Fas 4.5b).
