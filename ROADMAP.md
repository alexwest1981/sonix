...
| 2 | ~~**Plugins: VST3-vägen läser inga parametrar**~~ | *S–M* | **KLAR 2026-09-15.** Surge XT VST3: **0 → 2855 parametrar** (varav **774 automatiserbara** — exakt samma tal som CLAP-vägens 774, alltså en oberoende kvittens mellan två backends för samma plugin). Surge XT Effects: 0 → 14. **Den saknade biten var `IConnectionPoint`-kopplingen mellan komponenten och kontrollern** (`vst/ivstmessage.h`): VST3 delar en plugin i två halvor, och en JUCE-byggd kontroller (Surge XT:s VST3 är byggd med JUCE) får sin ljudprocessor — och bygger därmed sin parameterlista — först i `connect`. Fyra saker prövades innan dess, tre av dem negativa och alla mätta: vtabellen (identisk med headern), state-överföringen komponent→kontroller (50 239 byte kom över — utan effekt), en riktig `IHostApplication` i `initialize` (utan effekt), och *data-katalogen* (fullpaketet installerades: utan effekt). Det var **JUCE:s egen källkod** som pekade rätt, inte en femte gissning. Bonus på vägen: versionen kommer nu fram (`class_info2`/`IPluginFactory2`). **Låst i ett enhetsprov 2026-09-15:** `mock_vst3.c` implementerar nu `IConnectionPoint` på **båda** halvorna och svarar **noll** parametrar tills kopplingen gjorts — precis som en JUCE-byggd kontroller. `loads_mock_module_and_reports_parameters` kräver alltså handskakningen: med kopplingen avstängd i `open()` faller provet med `left: 0, right: 2`, samma symptom som Surge XT gav. På vägen föll också en **tyst grind**: ett felstavat typpnamn i fixturen kompilerade inte, `SONIX_MOCK_VST3` uteblev, och proven **hoppade över sig själva** (`option_env!` → `return`) och såg gröna ut — `build.rs` fäller nu bygget när kompilatorn finns men fixturen inte bygger, och skiljer det från en miljö utan `cc` | — |
| 3 | ~~**LV2: löftet i vyn har ingen värd bakom sig**~~ | *S* | **KLAR 2026-09-15** — löftet ströks, sanningen skrevs in. Skanningsraden `/usr/lib/lv2` är **avstängd** och beskrivs med orsaken ("ingen LV2-värd i Sonix än") i stället för att tyst försvinna; "LV2" är struket ur rubriken, importtipset och skanningsbeskrivningen; och `--inspect-plugin` på en `.lv2` (eller en fil inuti bunten) svarar **"LV2 stöds inte av Sonix än"** i stället för `undefined symbol: clap_entry` — ett svar som beskrev vår laddare och inte användarens fil. Tre prov: `is_lv2_path` (bunt, fil i bunt, och `.clap`/`.vst3` som *inte* är LV2), **ingen påslagen skanningsväg lovar LV2** (provet läser listan, inte texten — en omskriven beskrivning hade sett bra ut medan vägen fortfarande skannade) och engelskan för de fyra ändrade raderna. Mätt mot den riktiga Surge XT-bunten. **Att bygga en LV2-värd är fortfarande ogjort** — det är nu en egen punkt, inte ett brutet löfte | — |
...
- [ ] **8.6 Plugins: bryggning, egna utgångar och sidokedja in i en plugin** — *M*
  **Klar 2026-09-15** (fyra commits): `79adff9` (latens-offset + Smart disable), `6c1b43b`
  ...
  - ✅ **Mätt mot en riktig plugin 2026-09-15 (Surge XT 1.3.4).** Surge XT är GPL-3,
    ...
    - **LV2-löftet ströks 2026-09-15.** Se Kvar-tabellen: raden är avstängd med orsaken
      utskriven, texterna säger vad vi gör, och kommandoraden vägrar med sanningen i stället för
      med laddarens fel.
