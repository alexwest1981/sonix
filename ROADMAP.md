...
| 11 | **8.10 Ljudet följer tempot** *(2026-09-12)* | *M* | **Raden var inaktuell — rättad 2026-09-14.** Sträckningen ÄR inkopplad i tidslinjen och hela kedjan är kvitterad av örat (8.13b → 8.10b → 8.10c → 8.10d, se avsnitten). **Det som faktiskt återstår:** (1) **en kloss över ett tempobyte — `stretch_pieces` byggd 2026-09-14** (delar klippets ut-tid vid varje byte **i takter**, ett stycke per tempovärde; ett enda tempo ger exakt ett stycke = bit-identiskt med i dag). **Tempot läses nu där klippet ligger** (2026-09-14) på **alla tre levande ställen** — vågformens ritning, utsnittet och spara-som-sample — så en kloss *efter* ett byte följer bytet. **Ett stycke per tempovärde (2026-09-14):** beställningsvägen `ensure_stretched` går nu per stycke, så ett klipp som spänner över ett byte får **en sträckning per avsnitt** — `stretch_pieces` anropas, skulden är inlöst och allow-raden borta (0 varningar). Ett klipp inom ett enda tempo ger **exakt ett** stycke med dagens nyckel: ingen extra rendering, inget arbete ingen hör. **Kvar:** uppspelningen (de regioner som skickas till motorn) använder fortfarande en region per klipp — sträckningarna finns nu färdiga att peka på; (2) `sample_offset_sec > 0` kan hamna utanför sitt eget utsnitt (fel, egen rad); (3) **Alex' öra på A/B-filen — GJORT 2026-09-14:** *"båda har lite artefakter, men andra ljudet lät lite renare."* **Inget motorbyte:** signalsmith är ett dev-beroende (finns inte i binären) och smetar transienter (52 anslag av 24 klick). Fyndet: artefakterna sitter i **sustain**, inte i anslagen — det är där en förbättring ska riktas | — |
