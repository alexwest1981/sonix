...
| 2 | **Plugins: VST3-vägen läser inga parametrar** *(fynd 2026-09-15)* | *S–M* | **Halvvägs.** **Versionen är lagad** (`class_info2`): `--inspect-plugin` på `~/.vst3/Surge XT.vst3` svarar nu *"Surge Synth Team v**1.3.4**"* i stället för tomt — versionen finns bara i klassinfo **version 2**, som VST3 lägger i ett eget gränssnitt (`IPluginFactory2`, frågat med `queryInterface`), och vi läste bara `PClassInfo` där fältet inte finns. **Kvar:** parameterlistan — fortfarande **775 i CLAP, 0 i VST3**. Utrett och uteslutet: vtabellen (metod för metod identisk med den officiella headern: `getParameterCount` i slot 8, `getParameterInfo` i 9) och **state-överföringen** komponent→kontroller (införd, mätt: ändrade ingenting för Surge XT — den står kvar som protokollsteg med den negativa mätningen i doc-raden). **Fyra mätningar gjorda, tre negativa:** vtabellen är identisk med headern, state-överföringen komponent→kontroller ändrade ingenting, och **en riktig `IHostApplication` i `initialize` (i stället för `null`) ändrade inte heller talet** — alla tre står kvar som protokollsteg med sina negativa mätningar i doc-raderna. **Nästa mätning är instrumentering, inte en gissning:** skriv ut (bakom `SONIX_VST3_DEBUG`, som motorn redan har en sådan flagga) om `getState` finns, vad den returnerade, strömmens längd och `getParameterCount`s råa svar — då syns det om överföringen körde alls, i stället för att ännu en hypotes prövas blint | vid datorn, nu |
